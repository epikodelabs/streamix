...
const RESERVED_USERNAMES =
  new Set(['admin', 'angular', 'root', 'streamix']);
  ...
  return form({
    ...
    address: form({
      ...
      postalCode: field(initial.address.postalCode, {
        checks: [
          checks.required,
          checks.pattern(POSTAL_CODE_PATTERN),
        ],
      }),
    }),
...
export function createFieldViews(formState: ProfileForm) {
  ...
  return {
    profile: [
      ...
      view(profile.fields.username, 'Username', 'text', {
        pendingHint: 'Checking username availability...',
      }),
      view(profile.fields.bio, 'Bio', 'textarea', {
        rows: 4,
      }),
    ],
...
export function skillYearsView(skill: SkillForm): FieldView<number> {
  return view(skill.fields.years, 'Years', 'number', {
    min: 1,
    max: 20,
    compact: true,
  });
...
export function fieldHint(view: FieldView<any>): string | null {
  const { node, pendingHint, hint } = view;

  if (pendingHint && node.pending.value) {
    return pendingHint;
  }

  return hint?.(node.completeValue.value) ?? null;
}

export function fieldError(view: FieldView<any>): string | null {
  const { node, pendingHint } = view;

  if ((pendingHint && node.pending.value) || !node.touched.value) {
    return null;
  }
  ...
  const [name, payload] = Object.entries(node.issues.value ?? {})[0] ?? [];
  if (!name) {
    return null;
  }
...
export function cloneInitialProfile(): ProfileFormValue {
...
export function passwordMismatch(
  ...
  const { password, confirmPassword } =
    formState.fields.security.fields;
...
function syncSkills(
  ...
  while (skills.items.length < next.length) {
    skills.push(createSkill(next[skills.items.length]));
  }
}
...
function text(value: string, minimum = 1) {
  return field(value, {
    checks: minimum > 1
      ? [checks.required, checks.minLength(minimum)]
      : checks.required,
  });
}

function readRequired(payload: unknown): string {
