...
import { createHttpClient, readArrayBuffer, readBlob, readJson, readStatus, readText, useAccept, useBase, useFallback, useHeader, useLogger, useRedirect, useStripHeaders, useTimeout, } from '@epikodelabs/streamix/networking';
/* ─── Helpers ─── */
...
const test404 = async () => {
    ...
    const response$ = pipe(client
        .get('/nonexistent-endpoint-12345', readText), catchError((err: any) => { setOutput('404', `Caught 404: ${err.message || err}`); }));
    try {
...
const testTimeout = async () => {
    ...
    const response$ = pipe(client
        .get('/posts', readArrayBuffer), catchError((err: any) => { setOutput('timeout', `Caught timeout: ${err.message || err}`); }));
    try {
        for await (const value of response$) {
