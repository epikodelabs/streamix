...
describe('eachValueFrom', () => {
  ...
  it('should handle an empty source', async () => {
    ...
    expect(firstMessage).toBe('Stream completed without emitting a value');
    ...
    expect(lastMessage).toBe('Stream completed without emitting a value');
  ...
  it('should not treat undefined as completion', async () => {
    ...
    for await (const value of eachValueFrom(atom)) {
      values.push(value);
    }
