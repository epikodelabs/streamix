 ...
 * @param {Record<string, any>} params The data to send to the worker for computation.
 * @returns {Stream<T>} A new stream that emits the result of the computation.
 */
export function compute<T = any>(task: Coroutine, params: Record<string, any>): Stream<T> {
