// streamix-form-binding.directive.ts
import {
  AfterViewInit,
  Directive,
  ElementRef,
...
export class StreamixFormBindingDirective implements AfterViewInit, OnDestroy {
  @Input({ required: true }) sxFormBinding!: Form<any>;
  ...
  private bindForm(): void {
    ...
    // If host is a <form>, disable native validation
    if (root instanceof HTMLFormElement) {
      root.noValidate = true;
    } else {
      // Optionally find the nearest form and disable it
  ...
  private refresh(): void {
    ...
    root
      ...
      .forEach((element) => {
        ...
        // Already bound and still present — leave it alone. Previously this
        // method disposed and rebuilt *every* control on *any* childList
        // mutation. But bindControl's own render() causes error/hint <small>
        // elements to appear and disappear (e.g. while a field is `pending`),
        // which is itself a childList mutation of this subtree. That
        // re-triggered refresh(), which called useValidation() again on
        // every field, which restarts async checks and flips `pending` back
        // on, which mutates the DOM again — an infinite feedback loop. That
        // loop is what made the username field re-check itself repeatedly
        // and made typing into the password field hang the page.
        if (this.controls.has(element)) return;

        const field = resolveField(this.sxFormBinding, element.name);
        if (!field) return;
        this.controls.set(element, bindControl(element, field));
      });

    // Unbind controls whose elements were actually removed from the DOM
    // (e.g. a deleted skill row), so we don't leak listeners or validation
    // sources.
...
// --------------------------------------------------------------------------
// Private control binding (internal)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// DOM helpers (unchanged from your manual binding)
// --------------------------------------------------------------------------
